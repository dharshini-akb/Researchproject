...
print("Starting V2 targeted correction pass...")

# The 15 core literature references (2020-2026) exactly matching Table 1
core_refs_15 = [
...
# -------------------------------------------------------------------------
# 1. BUILD COMPLETE MARKDOWN (RareDXAI_Research_Manuscript_FINAL_v2.md)
# -------------------------------------------------------------------------
print("Generating reports/RareDXAI_Research_Manuscript_FINAL_v2.md...")

md_text = r"""# RareDXAI: A Human Phenotype Ontology-Based Framework for Rare Disease Prediction
...
md_v2_path = os.path.join(REPORTS_DIR, "RareDXAI_Research_Manuscript_FINAL_v2.md")
with open(md_v2_path, "w", encoding="utf-8") as f:
    f.write(md_text)
print(f"Markdown V2 document saved successfully to {md_v2_path}!")

# -------------------------------------------------------------------------
# 2. BUILD WORD DOCUMENT (RareDXAI_Research_Manuscript_FINAL_v2.docx)
# -------------------------------------------------------------------------
print("Generating reports/RareDXAI_Research_Manuscript_FINAL_v2.docx...")
...
add_p("On the held-out test set (N=78), RareDXAI correctly classified 77 out of 78 patients, corresponding to a held-out classification accuracy of 98.72% (Wilson 95% CI: 93.09%–99.77%), balanced accuracy of 96.30%, macro precision of 99.45%, macro recall of 96.30%, macro specificity of 98.15%, and macro F1-score of 97.76%. Multi-class threshold-agnostic evaluation on this held-out test set yielded a macro AUROC of 1.0000 and macro AUPRC of 1.0000, with a multiclass Brier score of 0.0419 and Expected Calibration Error (ECE) of 8.59%. Five-fold stratified grouped cross-validation demonstrated high consistency (97.14% ± 2.23% accuracy; macro F1: 94.89% ± 4.04%). In contrast, source-publication-grouped stress validation (GroupKFold by source publication) showed lower performance (79.62% ± 27.05% accuracy; macro F1: 59.51% ± 26.81%), indicating that publication-specific phenotype reporting patterns affect model transferability. TreeSHAP analysis identified standardized HPO features strongly associated with model decision boundaries, such as macrodontia (HP:0001572) for KBG syndrome, autism spectrum traits (HP:0000717) for White-Sutton syndrome, and thin upper lip vermilion (HP:0000219) for Xia-Gibbs syndrome. Exploratory upstream optical character recognition (OCR) on digitized notes achieved a Character Error Rate of 11.42% and Word Error Rate of 92.00%.", bold_prefix="Results: ")
...
add_h2("6.3 Cross-Validation vs. Source-Publication-Grouped Stress Validation")
...
add_p("2. Source-Publication-Grouped Stress Validation (GroupKFold by Source): Cross-validation was grouped strictly by primary source publication, withholding entire publications from training folds: Mean Accuracy of 79.62% ± 27.05% and Mean Macro F1 of 59.51% ± 26.81%.")

add_image_figure("figure6_cross_validation_performance.png", "Figure 7. Comparison between profile-grouped 5-fold cross-validation and source-publication-grouped stress validation. Profile-grouped CV achieved 97.14% ± 2.23% accuracy, whereas source-publication-grouped stress validation dropped to 79.62% ± 27.05% accuracy, indicating that publication-specific reporting habits influence model transferability.", width_inches=5.5)

add_p("The source-publication-grouped stress validation suggests that performance may decrease when the model encounters phenotype distributions and reporting patterns from previously unseen publications.")
...
add_p("3. Source-Publication-Grouped Performance Drop: The drop to 79.62% accuracy in source-publication-grouped validation indicates that differences in clinical reporting styles affect model transferability.")
...
# 8. References (Exactly 15 verified studies from 2020-2026 matching Table 1)
add_h1("8. References")
for ref in core_refs_15:
...
docx_v2_path = os.path.join(REPORTS_DIR, "RareDXAI_Research_Manuscript_FINAL_v2.docx")
doc.save(docx_v2_path)
print(f"Editable Word V2 document saved successfully to {docx_v2_path}!")

# -------------------------------------------------------------------------
# 3. BUILD PDF DOCUMENT (RareDXAI_Research_Manuscript_FINAL_v2.pdf)
# -------------------------------------------------------------------------
print("Generating reports/RareDXAI_Research_Manuscript_FINAL_v2.pdf...")

pdf_v2_path = os.path.join(REPORTS_DIR, "RareDXAI_Research_Manuscript_FINAL_v2.pdf")
...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_decorations(self, page_count):
        ...
        if self._pageNumber > 1:
            self.drawString(54, 750, "RareDXAI: HPO-Based Rare Disease Prediction • Research Manuscript")
        ...
        self.drawString(54, 32, "Confidential • Computational Genomics & Clinical AI • Locked Evidence Package (N = 385)")
...
p_title = ParagraphStyle('PDFTitle', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=16, leading=20, alignment=TA_CENTER, textColor=colors.HexColor("#1E3A8A"), spaceAfter=6)
p_authors = ParagraphStyle('PDFAuthors', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=10.5, leading=14, alignment=TA_CENTER, textColor=colors.HexColor("#0F172A"), spaceAfter=3)
p_affil = ParagraphStyle('PDFAffil', parent=styles['Normal'], fontName='Helvetica-Oblique', fontSize=9, leading=12, alignment=TA_CENTER, textColor=colors.HexColor("#475569"), spaceAfter=12)
p_h1 = ParagraphStyle('PDFH1', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=13, leading=16, textColor=colors.HexColor("#1E3A8A"), spaceBefore=14, spaceAfter=5, keepWithNext=True)
p_h2 = ParagraphStyle('PDFH2', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=10.5, leading=13, textColor=colors.HexColor("#0D9488"), spaceBefore=10, spaceAfter=3, keepWithNext=True)
p_body = ParagraphStyle('PDFBody', parent=styles['Normal'], fontName='Helvetica', fontSize=9.5, leading=13.5, alignment=TA_JUSTIFY, textColor=colors.HexColor("#0F172A"), spaceAfter=5)
p_callout = ParagraphStyle('PDFCallout', parent=styles['Normal'], fontName='Helvetica-Oblique', fontSize=8.5, leading=11.5, textColor=colors.HexColor("#334155"))
p_cap = ParagraphStyle('PDFCaption', parent=styles['Normal'], fontName='Helvetica-Oblique', fontSize=8.5, leading=11.5, alignment=TA_CENTER, textColor=colors.HexColor("#475569"), spaceBefore=3, spaceAfter=8)
p_tbl_hdr = ParagraphStyle('PDFTblHdr', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=8, leading=10, alignment=TA_CENTER, textColor=colors.white)
p_tbl_cell = ParagraphStyle('PDFTblCell', parent=styles['Normal'], fontName='Helvetica', fontSize=7.5, leading=9.5, alignment=TA_CENTER, textColor=colors.HexColor("#0F172A"))
p_tbl_cell_l = ParagraphStyle('PDFTblCellL', parent=styles['Normal'], fontName='Helvetica', fontSize=7.5, leading=9.5, alignment=TA_LEFT, textColor=colors.HexColor("#0F172A"))
p_ref = ParagraphStyle('PDFRef', parent=styles['Normal'], fontName='Helvetica', fontSize=8, leading=11, leftIndent=12, textColor=colors.HexColor("#1E293B"), spaceAfter=3)

story = []

# Title & Metadata
story.append(Paragraph("RareDXAI: A Human Phenotype Ontology-Based Framework for Rare Disease Prediction", p_title))
story.append(Paragraph("Dharshini K. et al.", p_authors))
story.append(Paragraph("Computational Genomics & Clinical AI Research Group<br/>Official Research Manuscript • Locked & Audited Evidence Package (N = 385)", p_affil))
story.append(HRFlowable(width="100%", thickness=1, color=colors.HexColor("#CBD5E1"), spaceAfter=10))

# 1. Abstract
story.append(Paragraph("1. Abstract", p_h1))
story.append(Paragraph("<b>Background:</b> Rare genetic diseases affect more than 300 million people worldwide. Diagnosing these conditions is challenging because many distinct genetic disorders present with overlapping clinical features, such as developmental delays, speech impairments, and intellectual disability. In pediatric genetics, syndromic neurodevelopmental conditions like KBG syndrome, White-Sutton syndrome, and Xia-Gibbs syndrome share non-specific clinical signs that frequently lead to prolonged diagnostic delays lasting five to seven years.", p_body))
story.append(Paragraph("<b>Methods:</b> We developed <b>RareDXAI</b>, a computational decision-support framework that uses standardized Human Phenotype Ontology (HPO) terms to perform phenotype-based classification among three predefined rare syndromes. We curated an audited, literature-derived cohort of 385 patients with confirmed pathogenic variants across 48 peer-reviewed publications (from 52 candidate literature sources evaluated during curation, with four secondary, overlapping, or re-reported sources containing 59 candidate records quarantined under a predefined exclusion protocol). Clinical features were encoded into 78 binary HPO terms and 3 one-hot sex indicators (81 total predictor dimensions). To reduce the risk of phenotype-profile leakage, identical phenotypic profiles were grouped before applying a 60/20/20 train/validation/test split (N=230 training, N=77 validation, N=78 held-out test), and feature vocabularies were constructed strictly from training data. We trained a Random Forest model with Platt probability scaling (5-fold internal cross-calibration) and explained predictions using TreeSHAP.", p_body))
story.append(Paragraph("<b>Results:</b> On the held-out test set (N=78), RareDXAI correctly classified 77 out of 78 patients, corresponding to a held-out classification accuracy of <b>98.72%</b> (Wilson 95% CI: 93.09%–99.77%), balanced accuracy of <b>96.30%</b>, macro precision of <b>99.45%</b>, macro recall of <b>96.30%</b>, macro specificity of <b>98.15%</b>, and macro F1-score of <b>97.76%</b>. Multi-class threshold-agnostic evaluation on this held-out test set yielded a macro AUROC of <b>1.0000</b> and macro AUPRC of <b>1.0000</b>, with a multiclass Brier score of <b>0.0419</b> and Expected Calibration Error (ECE) of <b>8.59%</b>. Five-fold stratified grouped cross-validation demonstrated high consistency (97.14% ± 2.23% accuracy; macro F1: 94.89% ± 4.04%). In contrast, source-publication-grouped stress validation (GroupKFold by source publication) showed lower performance (79.62% ± 27.05% accuracy; macro F1: 59.51% ± 26.81%), indicating that publication-specific phenotype reporting patterns affect model transferability. TreeSHAP analysis identified standardized HPO features strongly associated with model decision boundaries, such as macrodontia (HP:0001572) for KBG syndrome, autism spectrum traits (HP:0000717) for White-Sutton syndrome, and thin upper lip vermilion (HP:0000219) for Xia-Gibbs syndrome. Exploratory upstream optical character recognition (OCR) on digitized notes achieved a Character Error Rate of 11.42% and Word Error Rate of 92.00%.", p_body))
story.append(Paragraph("<b>Conclusions:</b> RareDXAI provides an interpretable phenotype-based classification framework and a reproducible baseline for rare disease decision support. While held-out test classification performance is high under profile-grouped evaluation, the substantial performance drop in source-publication-grouped stress testing highlights that independent external and prospective clinical validation remain necessary before deployment.", p_body))
story.append(Paragraph("<b>Keywords:</b> Rare Diseases, Human Phenotype Ontology, Machine Learning, Random Forest, Model Interpretability, SHAP, Clinical Decision Support, Neurodevelopmental Disorders.", p_body))

# 2. Introduction
story.append(Paragraph("2. Introduction", p_h1))
story.append(Paragraph("Rare diseases are medical conditions that affect a small fraction of the population, typically defined as fewer than 1 in 2,000 individuals in Europe or fewer than 200,000 individuals in the United States. Although each individual disorder is rare, there are over 7,000 recognized rare genetic conditions, which together affect an estimated 300 to 400 million people globally.", p_body))
story.append(Paragraph("Most rare diseases have an underlying genetic cause and manifest during infancy or early childhood. Despite advances in high-throughput DNA sequencing—including whole-exome sequencing (WES) and whole-genome sequencing (WGS)—affected patients and their families frequently experience a prolonged and stressful diagnostic odyssey. On average, obtaining a correct diagnosis requires five to seven years, multiple specialist visits, and several initial misdiagnoses.", p_body))
story.append(Paragraph("A principal contributor to this diagnostic delay is clinical phenotypic overlap. Many genetic conditions share broad, non-specific neurodevelopmental symptoms, such as intellectual disability, motor milestone delays, speech impairments, and behavioral challenges. This diagnostic challenge is particularly pronounced among three syndromic neurodevelopmental conditions:", p_body))
story.append(Paragraph("1. <b>KBG Syndrome (MIM #148050):</b> Caused by heterozygous loss-of-function mutations or microdeletions in the <i>ANKRD11</i> gene on chromosome 16q24.3 (Martinez-Cayuelas et al., 2023). Characteristic clinical findings include unusually large upper front teeth (macrodontia of the central incisors), a triangular facial appearance, prominent eyebrows, short stature, skeletal hand differences (such as brachydactyly or short fifth fingers), and mild-to-moderate intellectual disability.", p_body))
story.append(Paragraph("2. <b>White-Sutton Syndrome (MIM #616364):</b> Caused by heterozygous <i>de novo</i> mutations in the <i>POGZ</i> gene on chromosome 1q21.3 (Assia Batzir et al., 2020). Common clinical features include developmental delay, intellectual disability, speech impairment, autism spectrum disorder traits, small head size (microcephaly), and distinctive facial features.", p_body))
story.append(Paragraph("3. <b>Xia-Gibbs Syndrome (MIM #615829):</b> Caused by heterozygous <i>de novo</i> truncating mutations in the <i>AHDC1</i> gene on chromosome 1p36.11 (Khayat et al., 2021). Hallmark signs include severe infantile hypotonia (low muscle tone), global developmental delay, expressive speech absence or delay, a broad forehead, downward-slanting palpebral fissures, a thin upper lip vermilion, structural brain anomalies, and sleep disturbances such as obstructive sleep apnea.", p_body))
...
story.append(Paragraph("To standardize clinical descriptions and make them computable, the biomedical community established the <b>Human Phenotype Ontology (HPO)</b> (Köhler et al., 2021; Gargano et al., 2024). The HPO provides a controlled vocabulary of standardized medical terms organized in a directed acyclic graph. For example, instead of recording free-text phrases like 'large central teeth,' clinicians and computational pipelines use the standardized concept <code>HP:0001572</code> (Macrodontia of central incisors). This structured vocabulary allows machine-learning algorithms to analyze patient phenotypic profiles systematically.", p_body))
story.append(Paragraph("However, applying machine learning to rare disease phenomics involves specific methodological considerations: cohort sizes for ultra-rare disorders are inherently limited; retrospective cohorts derived from published clinical literature may carry selection and reporting biases; if patients with identical phenotypic profiles are placed into both training and evaluation sets, evaluations can be corrupted by profile overlap; and complex machine-learning models require explainability mechanisms so that clinicians can examine which clinical features influence algorithmic predictions.", p_body))
story.append(Paragraph("To address these challenges, we built <b>RareDXAI</b>, a phenotype-based computational framework designed to support candidate prioritization among syndromic mimics. RareDXAI uses an audited, literature-derived cohort of 385 molecularly confirmed patients from 48 peer-reviewed publications, applies profile-grouped data partitioning to reduce profile leakage, uses calibrated Random Forest classification to provide probability estimates, and uses SHAP (SHapley Additive exPlanations) to identify features strongly associated with model decision boundaries.", p_body))

syn_img = os.path.join(FIGURES_DIR, "syndrome_phenotypes_illustration.jpg")
if os.path.exists(syn_img):
    story.append(KeepTogether([
        RLImage(syn_img, width=460, height=276),
        Paragraph("<i>Figure 1. Representative clinical and phenotypic features associated with KBG, White-Sutton, and Xia-Gibbs syndromes. The illustration is intended for visual context and does not represent all affected individuals.</i>", p_cap)
    ]))

# 3. Literature Survey
story.append(Paragraph("3. Literature Survey", p_h1))
story.append(Paragraph("Computational methods for rare disease analysis have advanced from manual expert tables to ontology-driven machine learning, natural language processing (NLP), and deep learning. Mining individual patient case reports from published literature and converting them into structured, computable datasets is a well-established practice in computational genetics.", p_body))
story.append(Paragraph("We reviewed 15 peer-reviewed studies published between 2020 and 2026 that directly inform the design of RareDXAI. These studies fall into three main areas:", p_body))
story.append(Paragraph("<b>3.1 Ontology and Computable Phenotype Standards:</b> Standardized ontologies provide the vocabulary for computational phenomics. Köhler et al. (2021) and Gargano et al. (2024) detailed the ongoing growth of the Human Phenotype Ontology (HPO) to over 16,000 terms, providing the canonical hp.obo vocabulary (Release 2026-06-23) used in this work. Jacobsen et al. (2022) published the GA4GH Phenopacket schema in <i>Nature Biotechnology</i> (standardized as ISO 4454:2022), creating an international format for sharing computable patient-level phenotypic and genomic data.", p_body))
story.append(Paragraph("<b>3.2 Literature-Derived Patient Datasets and Phenotype Extraction:</b> Extracting patient-level data from published literature to construct computable cohorts is a foundational methodology. Ladewig et al. (2023) introduced the Phenopacket Store in <i>Database</i>, curating thousands of computable case reports directly from published literature into structured Phenopackets. The Phenopacket Store provides the closest methodological precedent to our study, demonstrating that structured extraction of published case reports enables reproducible benchmarking. RareDXAI builds upon this precedent by focusing on a specific three-syndrome classification problem with binary feature encoding, probability calibration, and SHAP interpretability.", p_body))
story.append(Paragraph("Complementing literature curation, Birgmeier et al. (2020) developed AMELIE in <i>Science Translational Medicine</i>, using NLP and machine learning to parse published literature for Mendelian diagnosis. Feng et al. (2021) created PhenoTagger in <i>Bioinformatics</i>, combining deep learning and dictionary indexing for automated HPO recognition. Liu et al. (2020) developed Doc2HPO in <i>BMC Bioinformatics</i> for interactive phenotype curation from clinical text. Schuetz et al. (2023) evaluated automated phenotype extraction from case reports in the <i>Journal of Biomedical Informatics</i>, demonstrating that OCR error propagation requires careful verification.", p_body))
story.append(Paragraph("<b>3.3 Phenotype-Based Machine Learning and Disease Prioritization:</b> Computational tools use phenotype matching to prioritize candidate diseases and genes. Robinson et al. (2020) introduced LIRICAL in <i>The American Journal of Human Genetics</i>, using likelihood ratios across observed and excluded HPO terms to calculate diagnostic odds. Zhao et al. (2020) developed Phen2Gene in <i>Nucleic Acids Research</i> for phenotype-driven gene prioritization. Hsieh et al. (2022) presented GestaltMatcher in <i>Nature Genetics</i>, using deep convolutional neural networks to match facial dysmorphology from clinical photographs. Rönicke et al. (2020) reviewed rare disease clinical decision support systems in <i>Molecular and Cellular Pediatrics</i>, emphasizing the importance of probability calibration, leakage controls, and interpretability.", p_body))
story.append(Paragraph("Finally, clinical cohort characterizations provided the primary data sources for our target conditions: Martinez-Cayuelas et al. (2023) for KBG syndrome (<i>ANKRD11</i>), Assia Batzir et al. (2020) for White-Sutton syndrome (<i>POGZ</i>), and Khayat et al. (2021) for Xia-Gibbs syndrome (<i>AHDC1</i>).", p_body))

# Table 1 PDF
tbl1_data = [[Paragraph(f"<b>{h}</b>", p_tbl_hdr) for h in lit_headers]]
for r in lit_rows:
    tbl1_data.append([
        Paragraph(r[0], p_tbl_cell),
        Paragraph(r[1], p_tbl_cell_l),
        Paragraph(r[2], p_tbl_cell_l),
        Paragraph(r[3], p_tbl_cell_l),
        Paragraph(r[4], p_tbl_cell),
        Paragraph(r[5], p_tbl_cell),
        Paragraph(r[6], p_tbl_cell_l),
        Paragraph(r[7], p_tbl_cell_l)
    ])
t1 = Table(tbl1_data, colWidths=[30, 52, 68, 55, 55, 55, 95, 94])
...
story.append(KeepTogether([
    Paragraph("<b>Table 1. Literature Comparison Matrix (15 Peer-Reviewed Studies, 2020–2026)</b>", p_h2),
    t1,
    Spacer(1, 8)
]))

# 4. Mathematical Expression
story.append(Paragraph("4. Mathematical Expression", p_h1))
story.append(Paragraph("This section defines the mathematical expressions used in RareDXAI for patient representation, classification, calibration, evaluation metrics, and SHAP interpretability.", p_body))

def pdf_eq_box(title, eq_str, exp_str):
    content = [
        Paragraph(f"<b>{title}</b>", p_h2),
        Spacer(1, 2),
        Paragraph(f"<font color='#1E3A8A'><b>{eq_str}</b></font>", ParagraphStyle('EqStyle', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=10, leading=13, alignment=TA_CENTER)),
        Spacer(1, 2),
        Paragraph(f"<i>Explanation: {exp_str}</i>", p_callout),
        Spacer(1, 6)
    ]
    return KeepTogether(content)

story.append(pdf_eq_box("4.1 Patient Phenotype Vector", "x_i = [x_i1, x_i2, ..., x_id] ∈ {0, 1}^d", "Each patient i is represented as a feature vector x_i of length d = 81, containing 78 binary HPO features and 3 demographic sex indicators."))
story.append(pdf_eq_box("4.2 Binary HPO Feature Encoding", "x_ij ∈ {0, 1} = 1 if documented; 0 if absent/unreported", "For each clinical term j, a value of 1 indicates documented phenotype; 0 indicates absent or unmentioned."))
story.append(pdf_eq_box("4.3 Random Forest Probability Estimation", "P(y = c | x) = (1 / T) * Σ I(h_t(x) = c)", "The ensemble consists of T = 100 decision trees h_t(x). The estimated probability for class c is the proportion of trees voting for that class."))
story.append(pdf_eq_box("4.4 Predicted Disease Class", "ŷ = argmax_c P(y = c | x)", "The predicted class ŷ is the syndrome receiving the highest estimated probability among the three target conditions."))
story.append(pdf_eq_box("4.5 Classification Accuracy", "Accuracy = Correct Classifications / Total Evaluated Patients", "Accuracy measures the proportion of correctly classified patients across the evaluation cohort N."))
story.append(pdf_eq_box("4.6 Precision (Positive Predictive Value)", "Precision = TP / (TP + FP)", "Precision measures the proportion of patients assigned to a syndrome who truly have that condition."))
story.append(pdf_eq_box("4.7 Recall (Sensitivity)", "Recall = TP / (TP + FN)", "Recall measures the proportion of actual patients with a syndrome who were correctly identified by the model."))
story.append(pdf_eq_box("4.8 F1-Score", "F1 = 2 * (Precision * Recall) / (Precision + Recall)", "The F1-score is the harmonic mean of precision and recall, balancing sensitivity and positive predictive value."))
story.append(pdf_eq_box("4.9 Balanced Accuracy", "Balanced Accuracy = (1 / C) * Σ Recall_c", "Balanced accuracy is the unweighted arithmetic mean of recall across all C = 3 syndromes."))
story.append(pdf_eq_box("4.10 SHAP Additive Feature Attribution", "f_c(x) = φ0,c + Σ φj,c", "TreeSHAP decomposes the model prediction for class c into a base expectation φ0,c plus the additive sum of individual feature attributions φj,c."))

# 5. Model Methodologies
story.append(Paragraph("5. Model Methodologies", p_h1))
story.append(Paragraph("Figure 2 illustrates the overall system workflow of RareDXAI, from literature curation to calibrated prediction and SHAP interpretation.", p_body))

fig1_img = os.path.join(FIGURES_DIR, "figure1_architecture.png")
if os.path.exists(fig1_img):
    story.append(KeepTogether([
        RLImage(fig1_img, width=460, height=276),
        Paragraph("<i>Figure 2. RareDXAI system workflow diagram. The pipeline processes published clinical case reports through standardized HPO concept mapping, executes profile-grouped stratified splitting to reduce profile leakage, trains a calibrated Random Forest classifier with Platt scaling, and outputs calibrated multi-class probabilities alongside SHAP feature attributions.</i>", p_cap)
    ]))

story.append(Paragraph("5.1 Cohort Assembly and Literature Curation Protocol", p_h2))
story.append(Paragraph("Patient data were gathered through structured curation of peer-reviewed clinical genetics literature. In total, 52 literature sources were evaluated during curation.", p_body))
story.append(Paragraph("To maintain data integrity and avoid duplicate patient entries, we established a quarantine protocol:", p_body))
story.append(Paragraph("• <b>Secondary Review Compilations:</b> Low et al. (2016, <i>Lancet</i>) included a summary table (Table 2) compiling 32 previously published KBG cases. Because we directly extracted the primary discovery publications, all 32 duplicate review entries were quarantined.", p_body))
story.append(Paragraph("• <b>Overlapping Case Series:</b> Ockeloen et al. (2015) (20 candidate records) and Walz et al. (2015) (6 candidate records) were quarantined due to substantial cross-cohort patient overlap with Goldenberg et al. (2016).", p_body))
story.append(Paragraph("• <b>Re-reported Single Cases:</b> Low et al. (2017) (1 candidate record) was quarantined as a duplicate of an earlier UK cohort entry.", p_body))
story.append(Paragraph("Under this protocol, four secondary, overlapping, or re-reported sources containing 59 candidate records were quarantined under the predefined duplicate/secondary-source exclusion protocol. The final audited cohort contains 385 unique patients across 48 included peer-reviewed publications, all with molecularly confirmed pathogenic variants.", p_body))

fig2_img = os.path.join(FIGURES_DIR, "figure2_provenance_flowchart.png")
if os.path.exists(fig2_img):
    story.append(KeepTogether([
        RLImage(fig2_img, width=460, height=276),
        Paragraph("<i>Figure 3. Literature curation and cohort provenance flowchart. From 52 candidate literature sources evaluated during curation, 4 duplicate/secondary sources (59 candidate records) were quarantined, leaving 48 included publications and 385 verified patients.</i>", p_cap)
    ]))

# Table 2 PDF
tbl2_data = [[Paragraph(f"<b>{h}</b>", p_tbl_hdr) for h in prov_headers]]
for r in prov_rows:
    tbl2_data.append([
        Paragraph(r[0], p_tbl_cell_l),
        Paragraph(r[1], p_tbl_cell),
        Paragraph(r[2], p_tbl_cell),
        Paragraph(r[3], p_tbl_cell),
        Paragraph(r[4], p_tbl_cell_l)
    ])
t2 = Table(tbl2_data, colWidths=[120, 80, 80, 80, 144])
...
story.append(KeepTogether([
    Paragraph("<b>Table 2. Literature Provenance and Source Attribution</b>", p_h2),
    t2,
    Spacer(1, 8)
]))

story.append(Paragraph("5.2 Phenotype Standardization and Vocabulary Construction", p_h2))
story.append(Paragraph("Clinical features were mapped to standardized HPO concepts using <code>hp.obo</code> (Release 2026-06-23). A total of 82 unique HPO terms appeared across the complete 385-patient dataset.", p_body))
story.append(Paragraph("To maintain evaluation integrity, the feature vocabulary was fitted strictly on the training partition (N = 230), yielding 78 training HPO terms. Four HPO terms occurring only in the held-out test set (HP:0002121, HP:0001156, HP:0001508, HP:0002126) were treated as out-of-vocabulary (OOV) features and masked during transformation. Zero OOV terms occurred in the validation set. Biological sex was one-hot encoded into three binary indicators (MALE, FEMALE, UNKNOWN_SEX), resulting in 81 total machine learning predictor dimensions.", p_body))

story.append(Paragraph("5.3 Leakage-Control Procedures and Data Partitioning", p_h2))
story.append(Paragraph("A critical challenge in clinical machine learning is data leakage caused by identical phenotypic profiles appearing across training and test sets. In our cohort of 385 patients, 370 distinct phenotypic profiles were identified (15 patients shared identical symptom-and-sex combinations with another patient having the same condition).", p_body))
story.append(Paragraph("Leakage-control procedures were implemented through patient-level separation, grouping of identical phenotypic profiles, and training-only feature vocabulary construction.", p_body))
story.append(Paragraph("We applied a 60/20/20 stratified split: Training Set (N = 230 patients, 59.7%), Validation Set (N = 77 patients, 20.0%), and Held-Out Test Set (N = 78 patients, 20.3%). No patient ID and no phenotypic profile appears in more than one partition. However, patients from the same publication could appear across partitions; therefore, source-publication-grouped stress validation was also performed to evaluate publication-specific effects.", p_body))

story.append(Paragraph("5.4 Machine Learning Classification and Probability Calibration", p_h2))
story.append(Paragraph("The primary predictive model is an ensemble Random Forest Classifier (100 decision trees, maximum tree depth of 10, balanced class weights, random seed 42).", p_body))
story.append(Paragraph("Random Forest vote proportions provide probability-like scores that may not be well calibrated. To obtain calibrated probability estimates, the ensemble was calibrated using sigmoid (Platt) scaling through 5-fold internal cross-validation fitted strictly on the training partition (CalibratedClassifierCV).", p_body))
story.append(Paragraph("We also evaluated baseline models on the exact same splits: Standard Random Forest, Logistic Regression (L2 penalty), Support Vector Machine (RBF kernel, calibrated), Decision Tree, K-Nearest Neighbors (k=5), Gaussian Naive Bayes, and XGBoost.", p_body))

story.append(Paragraph("5.5 Explainability with TreeSHAP", p_h2))
story.append(Paragraph("To explain model decision boundaries, we applied TreeSHAP (shap.TreeExplainer). SHAP values quantify the marginal contribution of each HPO feature toward the model's predicted probability for each syndrome.", p_body))

story.append(Paragraph("5.6 Exploratory Upstream Optical Character Recognition (OCR)", p_h2))
story.append(Paragraph("To evaluate the feasibility of ingesting scanned medical records, an exploratory OCR pipeline was built using EasyOCR and fuzzy concept matching.", p_body))

# 6. Results
story.append(Paragraph("6. Results", p_h1))
story.append(Paragraph("The audited cohort contains 385 patients with confirmed genetic diagnoses. Table 3 presents the demographic distribution and phenotypic characteristics across the three syndromes.", p_body))

# Table 3 PDF
tbl3_data = [[Paragraph(f"<b>{h}</b>", p_tbl_hdr) for h in demo_headers]]
for r in demo_rows:
    tbl3_data.append([
        Paragraph(r[0], p_tbl_cell_l),
        Paragraph(r[1], p_tbl_cell),
        ...
        Paragraph(r[4], p_tbl_cell),
        Paragraph(r[5], p_tbl_cell),
        Paragraph(r[6], p_tbl_cell),
        Paragraph(r[7], p_tbl_cell_l)
    ])
t3 = Table(tbl3_data, colWidths=[90, 45, 55, 40, 40, 50, 75, 109])
...
story.append(KeepTogether([
    Paragraph("<b>Table 3. Clinical Cohort and Demographic Characteristics</b>", p_h2),
    t3,
    Spacer(1, 8)
]))

fig3_img = os.path.join(FIGURES_DIR, "figure3_cohort_distribution.png")
if os.path.exists(fig3_img):
    story.append(KeepTogether([
        RLImage(fig3_img, width=420, height=252),
        Paragraph("<i>Figure 4. Cohort distribution of the RareDXAI dataset. The literature-derived cohort contains 385 patients: 298 KBG syndrome cases (77.40%), 45 White-Sutton syndrome cases (11.69%), and 42 Xia-Gibbs syndrome cases (10.91%).</i>", p_cap)
    ]))

story.append(Paragraph("6.1 Held-Out Test Set Classification Performance", p_h2))
story.append(Paragraph("On the held-out test set (N = 78), the Calibrated Random Forest correctly classified 77 out of 78 patients, corresponding to a held-out classification accuracy of <b>98.72%</b> (Wilson 95% CI: 93.09%–99.77%).", p_body))

# Table 4 PDF
tbl4_data = [[Paragraph(f"<b>{h}</b>", p_tbl_hdr) for h in bench_headers]]
for r in bench_rows:
    tbl4_data.append([
        Paragraph(r[0], p_tbl_cell_l),
        Paragraph(r[1], p_tbl_cell),
        ...
        Paragraph(r[5], p_tbl_cell),
        Paragraph(r[6], p_tbl_cell),
        Paragraph(r[7], p_tbl_cell),
        Paragraph(r[8], p_tbl_cell)
    ])
t4 = Table(tbl4_data, colWidths=[110, 85, 45, 42, 42, 45, 42, 45, 48])
...
story.append(KeepTogether([
    Paragraph("<b>Table 4. Machine Learning Model Performance on Held-Out Test Set (N = 78)</b>", p_h2),
    t4,
    Spacer(1, 8)
]))

story.append(Paragraph("<b>Model Selection Rationale:</b> Although Standard Random Forest and Logistic Regression achieved nominally higher accuracy on this particular held-out split, the Calibrated Random Forest was selected as the primary model because the framework explicitly prioritizes probability calibration and interpretable nonlinear phenotype modeling. Its multiclass Brier score was 0.0419, and TreeSHAP provided feature-level explanations of model decision boundaries.", p_body))

fig4_img = os.path.join(FIGURES_DIR, "figure4_confusion_matrix.png")
if os.path.exists(fig4_img):
    story.append(KeepTogether([
        RLImage(fig4_img, width=380, height=304),
        Paragraph("<i>Figure 5. Held-out test set confusion matrix (N = 78 patients). The model correctly classified 60 of 60 KBG cases, 9 of 9 Xia-Gibbs cases, and 8 of 9 White-Sutton cases, with a single atypical White-Sutton patient classified as KBG syndrome.</i>", p_cap)
    ]))

story.append(Paragraph("6.2 Per-Class Classification Performance Breakdown", p_h2))

# Table 5 PDF
tbl5_data = [[Paragraph(f"<b>{h}</b>", p_tbl_hdr) for h in class_headers]]
for r in class_rows:
    tbl5_data.append([
        Paragraph(r[0], p_tbl_cell_l),
        Paragraph(r[1], p_tbl_cell),
...
t5 = Table(tbl5_data, colWidths=[105, 45, 40, 45, 65, 45, 45, 55, 59])
...
story.append(KeepTogether([
    Paragraph("<b>Table 5. Per-Class Classification Performance Breakdown (Calibrated Random Forest)</b>", p_h2),
    t5,
    Spacer(1, 8)
]))

story.append(Paragraph("<b>Error Analysis:</b> The single misclassified patient was <code>WhiteSutton_PT19</code>, an individual with White-Sutton syndrome who presented with severe developmental delay and dental crowding, but lacked documented behavioral autism features. The predicted probability distribution indicated substantial uncertainty between KBG and White-Sutton for this case (KBG: 47.04%, White-Sutton: 46.51%, Xia-Gibbs: 6.45%), demonstrating that the model output reflected ambiguity rather than an overconfident error.", p_body))

fig5_img = os.path.join(FIGURES_DIR, "figure5_classification_performance.png")
if os.path.exists(fig5_img):
    story.append(KeepTogether([
        RLImage(fig5_img, width=420, height=252),
        Paragraph("<i>Figure 6. Multi-class classification performance on the held-out test set (N = 78). Accuracy: 98.72%, Balanced Accuracy: 96.30%, Macro Precision: 99.45%, Macro Recall: 96.30%, Macro Specificity: 98.15%, and Macro F1-Score: 97.76%.</i>", p_cap)
    ]))

story.append(Paragraph("6.3 Cross-Validation vs. Source-Publication-Grouped Stress Validation", p_h2))
story.append(Paragraph("To evaluate model consistency across different cohort partitions, we conducted two cross-validation experiments:", p_body))
story.append(Paragraph("1. <b>Profile-Grouped 5-Fold Cross-Validation:</b> Patients with identical profiles were retained within the same fold: Mean Accuracy of <b>97.14% ± 2.23%</b> (Fold scores: 0.9740, 0.9351, 0.9870, 1.0000, 0.9610) and Mean Macro F1 of <b>94.89% ± 4.04%</b> (Fold scores: 0.9572, 0.8851, 0.9776, 1.0000, 0.9246).", p_body))
story.append(Paragraph("2. <b>Source-Publication-Grouped Stress Validation (GroupKFold by Source):</b> Cross-validation was grouped strictly by primary source publication, withholding entire publications from training folds: Mean Accuracy of <b>79.62% ± 27.05%</b> and Mean Macro F1 of <b>59.51% ± 26.81%</b>.", p_body))

fig6_img = os.path.join(FIGURES_DIR, "figure6_cross_validation_performance.png")
if os.path.exists(fig6_img):
    story.append(KeepTogether([
        RLImage(fig6_img, width=420, height=252),
        Paragraph("<i>Figure 7. Comparison between profile-grouped 5-fold cross-validation and source-publication-grouped stress validation. Profile-grouped CV achieved 97.14% ± 2.23% accuracy, whereas source-publication-grouped stress validation dropped to 79.62% ± 27.05% accuracy, indicating that publication-specific reporting habits influence model transferability.</i>", p_cap)
    ]))

story.append(Paragraph("The source-publication-grouped stress validation suggests that performance may decrease when the model encounters phenotype distributions and reporting patterns from previously unseen publications.", p_body))

story.append(Paragraph("6.4 Model Interpretability with TreeSHAP", p_h2))
story.append(Paragraph("TreeSHAP analysis identified the standardized HPO features that most strongly influenced model decision boundaries. Table 6 lists the top 10 HPO features ranked by mean absolute SHAP value.", p_body))

# Table 6 PDF
tbl6_data = [[Paragraph(f"<b>{h}</b>", p_tbl_hdr) for h in shap_headers]]
for r in shap_rows:
    tbl6_data.append([
        Paragraph(r[0], p_tbl_cell),
        Paragraph(r[1], p_tbl_cell),
        ...
        Paragraph(r[3], p_tbl_cell),
        Paragraph(r[4], p_tbl_cell),
        Paragraph(r[5], p_tbl_cell_l)
    ])
t6 = Table(tbl6_data, colWidths=[28, 62, 110, 85, 65, 154])
...
story.append(KeepTogether([
    Paragraph("<b>Table 6. Key HPO Features Associated with Model Decision Boundaries</b>", p_h2),
    t6,
    Spacer(1, 8)
]))

story.append(Paragraph("<b>Note on Interpretation:</b> These HPO features were strongly associated with model decision boundaries. These features represent statistical associations with model decision boundaries and must not be interpreted as independent causal or definitive diagnostic criteria. The clinical context provided is background clinical interpretation, not a causal conclusion from SHAP.", p_body))

fig7_img = os.path.join(FIGURES_DIR, "figure7_shap_feature_importance.png")
if os.path.exists(fig7_img):
    story.append(KeepTogether([
        RLImage(fig7_img, width=460, height=276),
        Paragraph("<i>Figure 8. Standardized HPO features strongly associated with model decision boundaries. Features are ranked by mean absolute SHAP values across test patients.</i>", p_cap)
    ]))

story.append(Paragraph("6.5 Probability Calibration and Reliability Assessment", p_h2))
story.append(Paragraph("Probability calibration was assessed using Brier score and expected calibration error to evaluate the alignment between predicted probabilities and empirical outcomes: Multiclass Brier Score was <b>0.0419</b> (measures squared probability error; lower is better), Mean One-vs-Rest Brier Score was <b>0.0140</b> (White-Sutton: 0.0137; Xia-Gibbs: 0.0084; KBG: 0.0198), and Expected Calibration Error (ECE) was <b>8.59%</b> across 10 probability bins.", p_body))

fig8_img = os.path.join(FIGURES_DIR, "figure8_calibration_curve.png")
if os.path.exists(fig8_img):
    story.append(KeepTogether([
        RLImage(fig8_img, width=460, height=230),
        Paragraph("<i>Figure 9. Probability calibration and reliability assessment on the held-out test set (N = 78). (A) Multi-class reliability curve comparing predicted probability against empirical accuracy (ECE = 8.59%). (B) Multiclass and One-vs-Rest Brier score breakdown.</i>", p_cap)
    ]))

story.append(Paragraph("6.6 Threshold-Agnostic Performance (ROC and PR Curves)", p_h2))
story.append(Paragraph("Threshold-agnostic discrimination on the held-out test set yielded macro AUROC and macro AUPRC values of 1.0000: Macro AUROC was <b>1.0000</b> (White-Sutton: 1.0000; Xia-Gibbs: 1.0000; KBG: 1.0000) and Macro AUPRC was <b>1.0000</b> (White-Sutton: 1.0000; Xia-Gibbs: 1.0000; KBG: 1.0000). These values reflect strong separation on the held-out test set of 78 patients; however, the relatively small sample size and closed-set design limit broader interpretation.", p_body))

fig9_img = os.path.join(FIGURES_DIR, "figure9_roc_pr_curves.png")
if os.path.exists(fig9_img):
    story.append(KeepTogether([
        RLImage(fig9_img, width=460, height=230),
        Paragraph("<i>Figure 10. Discriminative performance curves across decision thresholds on the held-out test set (N = 78). (A) Multi-class One-vs-Rest ROC curves (Macro AUROC = 1.0000). (B) Multi-class Precision-Recall curves (Macro AUPRC = 1.0000).</i>", p_cap)
    ]))

story.append(Paragraph("6.7 Exploratory Upstream OCR Evaluation", p_h2))
story.append(Paragraph("Evaluation of the upstream EasyOCR module on scanned clinical summaries yielded Character Error Rate (CER) of <b>11.42%</b>, Word Error Rate (WER) of <b>92.00%</b>, Concept Mapping on Clean Text of <b>100.0%</b> (10/10 test phrases successfully mapped), and Concept Mapping on Noisy Scans of <b>80.0%</b> (8/10 test phrases successfully mapped).", p_body))
story.append(Paragraph("<b>Clinical Note:</b> Due to the high Word Error Rate on scanned documents, OCR is designated strictly as an exploratory upstream utility and is not clinically validated for automated standalone use without clinician verification.", p_body))

story.append(Paragraph("6.8 Limitations and Generalization Considerations", p_h2))
story.append(Paragraph("We acknowledge the following scientific limitations:", p_body))
story.append(Paragraph("1. <b>Moderate Sample Size:</b> Although N = 385 is substantial for these ultra-rare conditions, smaller sample sizes in minority classes (n = 45 for White-Sutton, n = 42 for Xia-Gibbs) yield wider confidence intervals.", p_body))
story.append(Paragraph("2. <b>Natural Class Imbalance:</b> KBG syndrome accounts for 77.40% of the cohort, reflecting higher historical publication volume rather than true epidemiological prevalence.", p_body))
story.append(Paragraph("3. <b>Source-Publication-Grouped Performance Drop:</b> The drop to 79.62% accuracy in source-publication-grouped validation indicates that differences in clinical reporting styles affect model transferability.", p_body))
story.append(Paragraph("4. <b>Literature Selection Bias:</b> Published case reports often emphasize classic or severe presentations, which may not fully represent milder community presentations.", p_body))
story.append(Paragraph("5. <b>No Prospective Hospital EHR Validation:</b> The framework was evaluated on retrospective literature cohorts; prospective validation in uncurated hospital electronic health records is still required.", p_body))
story.append(Paragraph("6. <b>Binary Coding of Symptoms:</b> Symptoms not mentioned in published reports are coded as 0, which cannot distinguish between true biological absence and clinical non-reporting.", p_body))
story.append(Paragraph("7. <b>Fixed Feature Vocabulary:</b> Four rare symptoms in the test set were unobserved during training and were masked out during inference.", p_body))
story.append(Paragraph("8. <b>Exploratory OCR Status:</b> The OCR pipeline is an exploratory upstream tool requiring manual human verification.", p_body))
story.append(Paragraph("9. <b>Closed-Set Scope:</b> The current model is a closed-set three-class classification system and should not be interpreted as a general-purpose rare disease diagnostic model.", p_body))
story.append(Paragraph("10. <b>Ontology Dependence:</b> Model accuracy depends directly on how accurately clinical findings are converted into standardized HPO codes.", p_body))
story.append(Paragraph("Therefore, the present study should be interpreted as a retrospective, literature-derived computational validation rather than evidence of clinical deployment readiness.", p_body))

# 7. Conclusion
story.append(Paragraph("7. Conclusion", p_h1))
story.append(Paragraph("This study presented RareDXAI, a computational framework that converts clinical genetics literature into structured Human Phenotype Ontology representations to assist in predicting and prioritizing between three syndromic neurodevelopmental disorders: KBG syndrome, White-Sutton syndrome, and Xia-Gibbs syndrome.", p_body))
story.append(Paragraph("By curating an audited cohort of 385 molecularly confirmed patients across 48 peer-reviewed publications and applying profile-grouped partitioning, RareDXAI achieved a held-out test classification accuracy of <b>98.72%</b> (macro F1 of <b>0.9776</b>) with calibrated probability estimation (Brier score of <b>0.0419</b>) and interpretable SHAP feature attributions.", p_body))
story.append(Paragraph("Importantly, source-publication-grouped stress validation demonstrated that performance decreases (79.62%) when evaluating phenotype distributions from previously unseen publications. This finding underscores that practical clinical decision support will require standardized phenotyping protocols and independent external validation. RareDXAI provides a transparent, reproducible baseline for phenotype-based rare disease decision support, establishing a foundation for future integration with genomic sequencing and electronic health records.", p_body))

# 8. References (Exactly 15 verified studies from 2020-2026 matching Table 1)
story.append(Paragraph("8. References", p_h1))
for ref in core_refs_15:
    story.append(Paragraph(ref, p_ref))

pdf_doc.build(story, canvasmaker=NumberedCanvas)
print(f"Professional PDF V2 document saved successfully to {pdf_v2_path}!")

# -------------------------------------------------------------------------
# 4. GENERATE V2 QUALITY CONTROL REPORT (RareDXAI_FINAL_V2_QC.md)
# -------------------------------------------------------------------------
print("Writing reports/RareDXAI_FINAL_V2_QC.md...")

v2_qc_content = """# RareDXAI: Final V2 Quality Control & Verification Audit Report
...
with open(os.path.join(REPORTS_DIR, "RareDXAI_FINAL_V2_QC.md"), "w", encoding="utf-8") as f:
    f.write(v2_qc_content)

print("V2 QC audit report saved successfully!")
print("\nALL V2 DELIVERABLES GENERATED SUCCESSFULLY!")
