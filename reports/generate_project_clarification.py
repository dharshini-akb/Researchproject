...
def generate_clarification_pdf(output_path):
    ...
    metadata_table_data = [
        ...
        [Paragraph("<b>Validation Status:</b>", body_style), Paragraph("Calibrated Random Forest achieves 97.78% accuracy on synthetic evaluation cohorts.", body_style)],
    ]
    ...
    story.append(Paragraph("• <b>Cornelia de Lange syndrome 1</b> (<i>OMIM:122470</i>) — A multisystem malformation syndrome characterized by dysmorphic facial features, growth retardation, and cognitive impairment.", bullet_style))
    story.append(Paragraph(
    ...
    story.append(Paragraph(
        "<b>Synthetic Patient Cohort:</b> Because real clinical cohorts for rare syndromes are highly scarce, a synthetic patient generator was implemented "
        "to synthesize realistic cohorts based on the official HPO frequency data. For each disease (White-Sutton, Xia-Gibbs, Cornelia de Lange), "
        "phenotypic term frequencies are translated into probability distributions. Synthetic profiles are then generated by sampling from these distributions, "
        "ensuring biological plausibility. The complete cohort is split into partitioned train, validation, and test datasets. The HPO vocabulary "
        "is built exclusively from the training dataset to prevent semantic leakage.",
        body_style
    ))
    story.append(Paragraph(
        "<i>Scientific Clarification: The system's current evaluations and reported results are based entirely on a SYNTHETIC HPO-DERIVED COHORT. "
        "It should not be described as real patient data.</i>",
        italic_body_style
    ))
    story.append(Spacer(1, 10))
    ...
    story.append(Paragraph("1. <b>Strict Split Isolation:</b> The synthetic cohort is divided into train (70%), validation (15%), and test (15%) partitions prior to any downstream transformation.", bullet_style))
    ...
    story.append(Paragraph("3. <b>Feature Overlap Audits:</b> The pipeline runs overlap checks during verification, ensuring zero patient profile duplicates exist between partitions (e.g., Train-Val overlap = 0, Train-Test overlap = 0, Val-Test overlap = 0).", bullet_style))
