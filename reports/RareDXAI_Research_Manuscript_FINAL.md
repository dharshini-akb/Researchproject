...
$$x_{ij} \in \{0, 1\} = egin{cases} 1 & 	ext{if HPO term } j 	ext{ is documented in patient } i \ 0 & 	ext{if HPO term } j 	ext{ is absent or not reported} \end{cases}$$
...
$$	ext{Accuracy} = rac{\sum_{i=1}^N \mathbb{I}(\hat{y}_i = y_i)}{N} = rac{	ext{Number of Correct Classifications}}{	ext{Total Number of Evaluated Patients}}$$
